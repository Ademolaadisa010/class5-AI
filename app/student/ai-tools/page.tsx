...
// ── Prompts ─────────────────────────────────────────────────────────
function buildPrompt(tab: Tab, notes: string): string {
...
You are an expert tutor. Explain the following concept simply and clearly, as if teaching a bright 16-year-old.
Format your response using these exact section headers wrapped in double asterisks like **Simple Analogy**, **What It Actually Means**, **Why It Matters**, **Key Takeaway**.
Under each header, write 2-4 bullet points starting with a dash (-).
Avoid jargon. Be engaging and use real-world examples where possible.
...
`.trim();
  ...
  // quiz
  return `
You are an expert quiz generator. Based on the following notes, generate exactly 5 multiple-choice questions.

Return ONLY a valid JSON array. No markdown, no explanation, no code block — just the raw JSON array.
...
Example format:
[{"q":"What is X?","options":["A","B","C","D"],"answer":2}]
...
const XP_RULES: Record<Tab, number> = { summarize: 40, explain: 30, quiz: 0 };
...
function ErrorBanner({ msg, onClose }: { msg: string; onClose: () => void }) {
  return (
    <div className="flex items-start gap-3 p-4 rounded-xl mt-4"
      ...
      <span style={{ color: "#F87171", flexShrink: 0, marginTop: 1 }}><Icon d={ic.alert} size={16} /></span>
      <p className="text-sm flex-1" style={{ color: "#FCA5A5", fontFamily: "var(--font-dm)" }}>{msg}</p>
      <button onClick={onClose} className="text-red-400 hover:text-white transition-colors flex-shrink-0">
        <Icon d={ic.x} size={14} />
      </button>
    </div>
  );
    ...
    <div className="rounded-2xl p-5 mt-4"
      ...
      <div className="space-y-2.5">
        {lines.map((line, i) => {
          ...
          if (isSection) return (
            <p key={i} className="font-bold text-sm pt-2 first:pt-0"
...
function QuizBlock({ questions, onXP }: {
  ...
  const handleNext = () => {
    ...
    const isCorrect = picked === questions[current].answer;
    if (isLast) {
      setFinished(true);
      if (!xpGiven) {
  ...
  if (finished) {
    const finalScore = score + (picked === questions[questions.length - 1].answer ? 1 : 0);
    return (
      <div className="rounded-2xl p-6 mt-4 text-center space-y-4"
        ...
        <div className="flex items-center justify-center gap-2 text-sm font-bold"
          ...
          <Icon d={ic.star} size={16} /> {finalScore * 20} XP earned!
        </div>
        ...
        {q.options.map((opt, i) => {
          ...
          return (
            <button key={opt} onClick={() => handlePick(i)}
              ...
              style={{ background: bg, border: `1px solid ${border}`, color, fontFamily: "var(--font-dm)", cursor: picked !== null ? "default" : "pointer" }}>
...
// ── Skeleton loader ───────────────────────────────────────────────────
function ResultSkeleton({ color }: { color: string }) {
  return (
    <div className="rounded-2xl p-5 mt-4 space-y-3"
      style={{ background: "rgba(30,41,59,0.7)", border: `1px solid ${color}25` }}>
      <div className="flex items-center gap-3 mb-4">
        <div className="w-6 h-6 rounded-lg animate-pulse" style={{ background: `${color}20` }} />
        <div className="h-4 w-32 rounded animate-pulse" style={{ background: "rgba(255,255,255,0.06)" }} />
        <div className="ml-auto h-4 w-20 rounded animate-pulse" style={{ background: "rgba(255,255,255,0.04)" }} />
      </div>
      {[80, 100, 60, 90, 70, 85].map((w, i) => (
        <div key={i} className="flex gap-2.5 items-start">
          <div className="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-2 animate-pulse" style={{ background: color }} />
          <div className="h-3.5 rounded animate-pulse flex-1" style={{ width: `${w}%`, background: "rgba(255,255,255,0.05)" }} />
        </div>
      ))}
    </div>
  );
...
export default function AIToolsPage() {
  ...
  // Sync XP from Firestore profile
  useEffect(() => {
    setLiveXP(profile?.xp ?? 0);
  }, [profile?.xp]);

  // ── Award XP to Firestore + local state ──────────────────────────
  ...
  // ── Increment usage counters in Firestore ────────────────────────
  const trackUsage = useCallback(async (t: Tab) => {
    if (!user) return;
    const field = t === "summarize" ? "summariesTotal" : t === "quiz" ? "quizzesTotal" : "explainsTotal";
    try {
      await updateDoc(doc(db, "users", user.uid), {
        [field]: increment(1),
      });
  ...
  const handleRun = async () => {
    ...
    try {
      const prompt = buildPrompt(tab, notes);
      const raw = await callGemini(prompt);

      if (tab === "quiz") {
        // Parse JSON quiz from Gemini
        const clean = raw
          .replace(/```json/gi, "")
          .replace(/```/g, "")
          .trim();
        ...
        try {
          ...
          else throw new Error("Could not parse quiz questions from AI response. Please try again.");
        }
        if (!Array.isArray(parsed) || parsed.length === 0) throw new Error("No quiz questions returned. Try with more detailed notes.");
        setQuizQuestions(parsed);
        setQuizMode(true);
        setResult("quiz-ready");
      } else {
        setResult(raw);
        await awardXP(XP_RULES[tab]);
      }

      await trackUsage(tab);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    setNotes(`[File: ${file.name}]\n\nFile uploaded. The AI will process this content — you can also add extra notes below.`);
  };
  ...
  return (
    <div className="px-5 py-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "#475569", fontFamily: "var(--font-dm)" }}>
            Powered by Gemini AI — summarize, explain, and quiz yourself to earn XP.
          </p>
        </div>
          ...
          <div key={t.id}
            ...
            <div className="mt-1 flex items-center justify-center gap-1">
              ...
              <span className="text-[10px] font-semibold" style={{ color: "#F59E0B", fontFamily: "var(--font-sora)" }}>
                {t.id === "quiz" ? "20 XP / correct" : `+${t.xp} XP`}
              </span>
            </div>
          </div>
        ))}
      ...
      <div className="rounded-2xl overflow-hidden"
        ...
        <div className="flex items-center gap-3 px-4 py-2.5"
          ...
          <button onClick={() => fileRef.current?.click()}
            ...
            style={{ background: `${tabCfg.color}14`, color: tabCfg.color, border: `1px solid ${tabCfg.color}25`, fontFamily: "var(--font-sora)" }}
            ...
            <Icon d={ic.pdf} size={13} /> Upload File
          </button>
        ...
        <textarea
          ...
          placeholder={
            tab === "summarize" ? "Paste your lecture notes, textbook chapter, or any text here…" :
            tab === "explain"   ? "Type a concept, topic, or paragraph you find confusing…" :
            "Paste your notes or topic content — Gemini will generate quiz questions from it…"
          }
          rows={8}
        ...
        <div className="flex items-center justify-between px-4 py-3"
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => { setNotes(SAMPLES[tab]); setResult(null); setQuizMode(false); setError(null); }}
            ...
            {notes && (
              <button onClick={() => { setNotes(""); setResult(null); setQuizMode(false); setError(null); }}
                className="text-xs flex items-center gap-1 transition-colors"
                style={{ color: "#475569", fontFamily: "var(--font-dm)" }}
          ...
          <button onClick={handleRun} disabled={!notes.trim() || loading}
      ...
      {loading && <ResultSkeleton color={tabCfg.color} />}

      {/* ── result ──────────────────────────────────────── */}
      {!loading && result && !quizMode && result !== "quiz-ready" && (
        <ResultBlock tab={tab} result={result} onCopy={handleCopy} copied={copied} />
      )}

      {/* ── quiz ────────────────────────────────────────── */}
      {!loading && quizMode && quizQuestions.length > 0 && (
        <div>
          ...
          <QuizBlock questions={quizQuestions} onXP={awardXP} />
        </div>
      )}
      ...
      {!result && !loading && (
        <div className="mt-6 grid sm:grid-cols-3 gap-3">
          {[
            ...
            { icon: ic.trophy,  color: "#F59E0B", tip: "Score 5/5 on a quiz to maximise your XP reward" },
